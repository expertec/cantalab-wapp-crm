...
/**
 * Envía nota de voz (audio) a un lead.
 * @param {string} phone  E.164 sin '+', ej. '521234567890'
 * @param {Buffer} buffer Audio convertido a OGG/Opus
 * @param {string} mimetype MIME p.ej. 'audio/ogg; codecs=opus'
 * @param {{ptt?: boolean}} [options]
 */
export async function sendAudioToLead(phone, buffer, mimetype, options = {}) {
    try {
      const sock = whatsappSock;
      if (!sock) throw new Error('No hay conexión activa con WhatsApp');
      ...
      // Guardar en Firebase como mensaje de salida
      const leadRef = db.collection('leads').doc(jid);
      const outMsg = {
        content: '',
        mediaType: 'audio',
      ...
      return { success: true, message: 'Audio enviado a WhatsApp' };
    } catch (error) {
      console.error("Error enviando audio a WhatsApp:", error);
      throw new Error(`Error enviando audio: ${error.message}`);
    }
  }